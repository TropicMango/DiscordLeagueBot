...
def get_champs(player):
    ...
    final = []
    for line in content:
        final.append("{}. {}|Strength: {}".format(i, line.split('|')[0], line.split('|')[1]))
        i += 1
    return final
...
def get_my_champ(player):
    ...
    final = ['{}_{}'.format(split_champ[0].split(' ')[0], split_champ[2][:-1]),
             'Strength: {} mangos'.format(split_champ[1])]
    return final
...
def change_mvp(player, ide):
    with open("./data/{}.txt".format(player), "r") as f:
        content = f.readlines()
    new_champ = content[int(ide) + 1].split(':')[0]
    with open("./data/{}.txt".format(player), "w") as f:
        for line in content:
